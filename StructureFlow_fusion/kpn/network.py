...
class KPN(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False, model_type = 2):
        ...
        self.conv_fusion = nn.Conv2d(in_channels=6, out_channels=3, kernel_size=1, stride=1)
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        if self.model_type == 2:
            pred1 = self.conv_fusion(pred1)
            pred2 = self.conv_fusion(pred2)
            pred3 = self.conv_fusion(pred3)
            pred4 = self.conv_fusion(pred4)
        ...
        # visualization(data_cp, core_cp, torch.clone(pred))
