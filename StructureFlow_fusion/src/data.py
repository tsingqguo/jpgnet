...
class Dataset(data.Dataset):
    ...
    def load_item(self, index):
        ...
        #dunhuang
        if self.mask_reverse == 1:
            inpaint_map = 1 - inpaint_map
    ...
    def load_mask(self, index, img):
        ...
        if self.mask_type == 'random_bbox':
            ...
            if self.transform_opt['random_load_mask']:
                index = np.random.randint(0, len(self.mask_image_files)-1)
                # index = 122
                ...
                index = index % len(self.mask_image_files)
                mask = gray_loader(self.mask_image_files[index])
            ...
            mask = transFunc.to_tensor(mask).float()
...
def gray_loader( path):
    img = crop_center(Image.open(path))
    return img
