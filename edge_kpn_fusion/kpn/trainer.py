...
def Pre_train(opt):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (true_input, true_target) in enumerate(train_loader):
            ...
            # Print log
            print("\r[train_Epoch %d/%d] [Batch %d/%d] [Loss: %.4f %.4f] Time_left: %s" %
        ...
        eval_cnt = 0
        generator.eval()
        for j, (true_input2, true_target2, height_origin2, width_origin2) in enumerate(test_loader):

            # To device
            if opt.no_gpu:
                true_input2 = true_input2
                true_target2 = true_target2
            else:
                true_input2 = true_input2.cuda()
                true_target2 = true_target2.cuda()
            ...
            # Save
            if epoch % len(test_loader) == j:
                img_list2 = [true_input2, fake_target2, true_target2]
                name_list2 = ['in', 'pred', 'gt']
                utils.save_sample_png(sample_folder=sample_folder, sample_name='%d' % (epoch + 1), img_list=img_list2,
                                     name_list=name_list2, pixel_max_cnt=255, height=height_origin2, width=width_origin2)
            ...
            img_pred_recover2 = utils.recover_process(fake_target2, height=height_origin2, width=width_origin2)
            img_gt_recover2 = utils.recover_process(true_target2, height=height_origin2, width=width_origin2)
            ...
            psnr_sum = psnr_sum + utils.psnr(img_pred_recover2, img_gt_recover2)
            ssim_sum = ssim_sum + compare_ssim(img_gt_recover2, img_pred_recover2, multichannel=True, data_range=255)
            eval_cnt = eval_cnt + 1

        psnr_ave = psnr_sum / eval_cnt
        ssim_ave = ssim_sum / eval_cnt

        print('eval_epoch:{} psnr_ave:{} ssim_ave:{}'.format(epoch+1, psnr_ave, ssim_ave))
