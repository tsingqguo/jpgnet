...
import  json
from kpn.dataset import get_data, RandomCrop
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, config, flist, edge_flist, mask_flist, augment=True, training=True):
        ...
        self.mask_threshhold = config.MASK_THRESHOLD
    ...
    def load_item(self, index):
        ...
        mask = self.load_mask(img, mask_index)
    ...
    def load_mask(self, img, index):
        ...
        # external
        if mask_type == 3 or mask_type == 6:
            mask = imread(self.mask_data[index])
            mask = self.resize(mask, imgh, imgw)
            mask = (mask > self.mask_threshhold).astype(np.uint8) * 255       # threshold due to interpolation
