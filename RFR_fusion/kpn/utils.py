...
#                 Network
# ----------------------------------------
def create_generator(opt, model_type, load_name):
    # Initialize the network
    generator = network.KPN(opt.color, opt.burst_length, opt.blind_est, opt.kernel_size, opt.sep_conv, \
        opt.channel_att, opt.spatial_att, opt.upMode, opt.core_bias, model_type)

    if load_name == '' or not os.path.isfile(load_name):
        # Init the network
        ...
        if torch.cuda.is_available():
            ...
            pretrained_net = torch.load(load_name, map_location='cpu')

        load_dict(generator, pretrained_net)
        print('Generator is loaded!__{}_{}'.format(load_name, model_type))

        if model_type == 2:
            iteration = load_name.split('/')[-1].split('_')[0]
...
def get_files(path):
    ...
    for root, dirs, files in os.walk(path):
        files.sort()    
        for name in files:
            file_rainy = path + "/" + name
            result.append(file_rainy)
