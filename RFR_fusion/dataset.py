...
from kpn.dataset import get_data
import config
from image_crop import crop_center
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, image_path, mask_path, mask_mode, target_size, augment=True, training=True, mask_reverse = False):
        ...
        print('train:{} mask size:{}'.format(self.training, len(self.mask_data)))
    ...
    def load_item(self, index):
        ...
        if self.training:
            img = self.resize(img)
            mask = self.load_mask(img, random.randint(0, len(self.mask_data) - 1))
        else:
        ...
        # img_name = self.data[index][-10:]
        # map['name'] = img_name
        # # kpn
        # img_rainy, img_gt, H, W = get_data(self.mask_data[index % len(self.mask_data)], self.data[index])



        # return self.to_tensor(img), self.to_tensor(mask),         img_rainy, img_gt, H, W
    ...
    def load_mask(self, img, index):
        ...
        if self.mask_type == 2:
            ...
            mask = imread(self.mask_data[mask_index])
            mask = self.resize(mask, False)
            mask = (mask > self.mask_threshold).astype(np.uint8)  # threshold due to interpolation
            if self.mask_reverse:
                return (1 - mask) * 255
            else:
                return mask * 255
